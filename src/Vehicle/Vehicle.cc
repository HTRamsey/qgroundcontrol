...
void Vehicle::_commonInit(LinkInterface* link)
{
    ...
    connect(_initialConnectStateMachine, &QStateMachine::finished,
            this, [this]() { _gotProgressUpdate(0.f); });
    // ParameterManager progress is now handled by InitialConnectStateMachine during initial connect.
    // After initial connect completes, parameter refresh progress updates are not shown.
