    ...
    , _terrainProtocolHandler       (new TerrainProtocolHandler(this, &_terrainFactGroup, this))
{
    ...
    qCDebug(VehicleLog) << "Link started with Mavlink " << (MAVLinkProtocol::instance()->getCurrentVersion() >= 2 ? "V2" : "V1");
...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    uint8_t mavlinkVersion = MAVLinkProtocol::instance()->getCurrentVersion();
    if ((_maxProtoVersion != mavlinkVersion) && (mavlinkVersion >= 2)) {
...
void Vehicle::_setMaxProtoVersion(uint8_t version) {
...
void Vehicle::_setMaxProtoVersionFromBothSources()
{
    if (_mavlinkProtocolRequestComplete && _capabilityBitsKnown) {
        if (_mavlinkProtocolRequestMaxProtoVersion != 0) {
            ...
            _setMaxProtoVersion(capabilityBits() & MAV_PROTOCOL_CAPABILITY_MAVLINK2 ? 2 : 1);
        }
    }
}
...
EventHandler& Vehicle::_eventHandler(uint8_t compid)
{
    ...
    if (eventData == _events.end()) {
        ...
        QSharedPointer<EventHandler> eventHandler{new EventHandler(this, profile,
                ...
                MAVLinkProtocol::instance()->getSystemId(), MAVLinkProtocol::instance()->getComponentId(), _id, compid)};
...
void Vehicle::_mavlinkMessageStatus(uint8_t sysid, uint64_t totalSent, uint64_t totalReceived, uint64_t totalLoss, float lossPercent)
{
    if(sysid == _id) {
...
void Vehicle::_sendSetupSigning()
{
    ...
    mavlink_message_t msg;
    (void) mavlink_msg_setup_signing_encode_chan(MAVLinkProtocol::instance()->getSystemId(), MAVLinkProtocol::instance()->getComponentId(), channel, &msg, &setup_signing);
