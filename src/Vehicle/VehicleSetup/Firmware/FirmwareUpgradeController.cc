    ...
    , _apmVehicleTypeSetting            (SettingsManager::instance()->firmwareUpgradeSettings()->apmVehicleType())
{
    ...
    _stateMachine = new FirmwareUpgradeStateMachine(this, this);

    // Connect thread controller signals to both state machine and controller slots
    ...
    // Connect state machine signals to QML
    connect(_stateMachine, &FirmwareUpgradeStateMachine::statusMessage, this, &FirmwareUpgradeController::_status);
    connect(_stateMachine, &FirmwareUpgradeStateMachine::progressChanged, this, [this](double progress) {
        if (_progressBar) {
            _progressBar->setProperty("value", static_cast<float>(progress));
        }
    });
    connect(_stateMachine, &FirmwareUpgradeStateMachine::flashComplete, this, [this](bool success, const QString& message) {
        Q_UNUSED(message)
        if (success) {
            emit flashComplete();
        } else {
...
void FirmwareUpgradeController::startBoardSearch(void)
{
    ...
    // Start board search via state machine
                                      ...
                                      FirmwareVehicleType_t vehicleType)
{
    ...
    // Get firmware URL for the specified identifier
    ...
    QString firmwareUrl = prgFirmware->value(firmwareId);
    if (firmwareUrl.isEmpty()) {
        _errorCancel(tr("No firmware file selected"));
        return;
    }

    _firmwareFilename = firmwareUrl;
    _stateMachine->startFlash(firmwareUrl);
}

void FirmwareUpgradeController::flashFirmwareUrl(QString firmwareFlashUrl)
{
    _firmwareFilename = firmwareFlashUrl;
    _stateMachine->startFlash(firmwareFlashUrl);
}
...
void FirmwareUpgradeController::_foundBoard(bool firstAttempt, const QSerialPortInfo& info, int boardType, QString boardName)
{
    ...
    _stateMachine->handleBoardFound(firstAttempt, info, boardType, boardName);
...
void FirmwareUpgradeController::_foundBoardInfo(int bootloaderVersion, int boardID, int flashSize)
{
    ...
    // Forward to state machine (it will decide whether to start flash or show firmware select)
    _stateMachine->handleBootloaderInfo(bootloaderVersion, boardID, flashSize);

    // Build APM firmware names if manifest is available
    if (_rgManifestFirmwareInfo.length()) {
        _buildAPMFirmwareNames();
    }

    // State machine will emit show_firmware_select if no pending flash
...
void FirmwareUpgradeController::_flashComplete(void)
{
    ...
    // Forward to state machine (which will emit flashComplete signal and allow connections)
    _stateMachine->handleFlashComplete();
}
...
void FirmwareUpgradeController::_error(const QString& errorString)
{
    ...
    _appendStatusLog(QString("Error: %1").arg(errorString), false);

    // Forward to state machine
    _stateMachine->handleError(errorString);
}

void FirmwareUpgradeController::_status(const QString& statusString)
{
    _appendStatusLog(statusString);
    _stateMachine->handleStatus(statusString);
}
...
void FirmwareUpgradeController::_updateProgress(int curr, int total)
{
    ...
    if (total > 0) {
        _progressBar->setProperty("value", static_cast<float>(curr) / static_cast<float>(total));
        _stateMachine->handleFlashProgress(curr, total);
    }
}
...
{
    _eraseTimer.stop();
    _stateMachine->handleEraseComplete();
}
