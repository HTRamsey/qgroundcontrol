...
QGC_LOGGING_CATEGORY(RemoteIDManagerLog, "qgc.vehicle.remoteidmanager")
...
RemoteIDManager::RemoteIDManager(Vehicle *vehicle)
    ...
    , _operatorIDGood(_settings->operatorIDValid()->rawValue().toBool())
    , _targetSystem(vehicle->id()) // Assign vehicle sysid and compid. GCS must target these messages to autopilot, and autopilot will redirect them to RID device
    , _targetComponent(vehicle->compId())
{
    ...
    (void) connect(&_odidTimeoutTimer, &QTimer::timeout, this, &RemoteIDManager::_odidTimeout);

    // Timer to send messages at a constant rate
    _sendMessagesTimer.setInterval(SENDING_RATE_MSEC);
    (void) connect(&_sendMessagesTimer, &QTimer::timeout, this, &RemoteIDManager::_sendMessages);

    // GCS GPS position updates to track the health of the GPS data
    (void) connect(QGCPositionManager::instance(), &QGCPositionManager::positionInfoUpdated, this, &RemoteIDManager::_updateLastGCSPositionInfo);

    // Check changes in basic id settings as long as they are modified
    (void) connect(_settings->basicID(), &Fact::rawValueChanged, this, &RemoteIDManager::_checkGCSBasicID);
    (void) connect(_settings->basicIDType(), &Fact::rawValueChanged, this, &RemoteIDManager::_checkGCSBasicID);
    (void) connect(_settings->basicIDUaType(), &Fact::rawValueChanged, this, &RemoteIDManager::_checkGCSBasicID);
...
void RemoteIDManager::mavlinkMessageReceived(const mavlink_message_t &message)
{
...
void RemoteIDManager::_handleArmStatus(const mavlink_message_t &message)
{
    ...
    if (armStatus.status == MAV_ODID_ARM_STATUS_PRE_ARM_FAIL_GENERIC) {
        ...
        if (armStatus.error == QStringLiteral("missing basic_id message")) {
            _basicIDGood = false;
...
const char *RemoteIDManager::_getSelfIDDescription()
{
    ...
    _descriptionBuffer = descriptionToSend.toLocal8Bit();
    _descriptionBuffer.resize(MAVLINK_MSG_OPEN_DRONE_ID_SELF_ID_FIELD_DESCRIPTION_LEN);
    return _descriptionBuffer.constData();
}
...
void RemoteIDManager::_sendOperatorID()
{
    ...
    mavlink_message_t msg{};
    (void) mavlink_msg_open_drone_id_operator_id_pack_chan(
        MAVLinkProtocol::instance()->getSystemId(),
        MAVLinkProtocol::getComponentId(),
        sharedLink->mavlinkChannel(),
        &msg,
...
void RemoteIDManager::_sendSystem()
{
    ...
    mavlink_message_t msg{};
    (void) mavlink_msg_open_drone_id_system_pack_chan(
        MAVLinkProtocol::instance()->getSystemId(),
        MAVLinkProtocol::getComponentId(),
        sharedLink->mavlinkChannel(),
        &msg,
...
{
    static constexpr uint32_t secsSinceEpoch2019 = 1546300800; // Secs elapsed since epoch to 1-1-2019
...
void RemoteIDManager::_sendBasicID()
{
    ...
    const QString basicIDTemp = _settings->basicID()->rawValue().toString();
    ...
    mavlink_message_t msg{};
    (void) mavlink_msg_open_drone_id_basic_id_pack_chan(
        MAVLinkProtocol::instance()->getSystemId(),
        MAVLinkProtocol::getComponentId(),
        sharedLink->mavlinkChannel(),
        &msg,
...
void RemoteIDManager::_checkGCSBasicID()
{
    ...
    _GCSBasicIDValid = (!basicID.isEmpty() && (_settings->basicIDType()->rawValue().toInt() >= 0) && (_settings->basicIDUaType()->rawValue().toInt() >= 0));
}
...
bool RemoteIDManager::_isEUOperatorIDValid(const QString &operatorID)
{
...
QChar RemoteIDManager::_calculateLuhnMod36(const QString &input)
{
    ...
    for (int i = input.length() - 1; i >= 0; i--) {
        const int codePoint = alphabet.indexOf(input[i]);
...
void RemoteIDManager::_updateLastGCSPositionInfo(const QGeoPositionInfo &update)
{
    if (update.isValid()) {
