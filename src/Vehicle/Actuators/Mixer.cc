        ...
        ParameterManager* parameterManager, std::function<void(Function, Fact*)> factAddedCb)
{
    ...
    } else if (parameterManager->parameterExists(ParameterManager::anyComponentId, param)) {
...
{
    if (!_parameterManager->parameterExists(ParameterManager::anyComponentId, paramName)) {
        qCDebug(MixerLog) << "Mixers: Param does not exist:" << paramName;
        return nullptr;
    }
    Fact* fact = _parameterManager->getParameter(ParameterManager::anyComponentId, paramName);
	subscribeFact(fact);
	return fact;
}
