...
#define PX4_FLOW_SYS_ID 81
#define PX4_FLOW_SYS_COMP_ID MAV_COMP_ID_USER26
#define PX4_FLOW_MAV_TYPE MAV_TYPE_GENERIC
#define PX4_FLOW_AUTOPILOT_TYPE MAV_AUTOPILOT_GENERIC
...
void MultiVehicleManager::_vehicleHeartbeatInfo(LinkInterface* link, uint8_t vehicleId, uint8_t componentId, MAV_AUTOPILOT vehicleFirmwareType, MAV_TYPE vehicleType)
{
    ...
    if (link->isPX4Flow()) {
        vehicleId = PX4_FLOW_SYS_ID;
        componentId = PX4_FLOW_SYS_COMP_ID;
        vehicleFirmwareType = PX4_FLOW_AUTOPILOT_TYPE;
        vehicleType = PX4_FLOW_MAV_TYPE;
    }

    if (componentId != MAV_COMP_ID_AUTOPILOT1) {
        // Special case for PX4 Flow
        if (vehicleId != PX4_FLOW_SYS_ID || componentId != PX4_FLOW_SYS_COMP_ID) {
