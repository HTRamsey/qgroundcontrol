...
QGC_LOGGING_CATEGORY(AutotuneLog, "qgc.vehicle.autotune")

Autotune::Autotune(Vehicle *vehicle)
    : QObject(vehicle)
    , _vehicle(vehicle)
{
    // qCDebug(AutotuneLog) << Q_FUNC_INFO << this;
...
void Autotune::ackHandler(void* resultHandlerData, int compId, const mavlink_command_ack_t &ack, Vehicle::MavCmdResultFailureCode_t failureCode)
{
    ...
    if (autotune->_autotuneInProgress) {
        ...
        qCWarning(AutotuneLog) << "Ack received for a command different from MAV_CMD_DO_AUTOTUNE_ENABLE ot wrong UI state.";
    }
}
