...
void APMSensorsComponentController::_startLogCalibration()
{
    ...
    (void) connect(_vehicle, &Vehicle::textMessageReceived, this, &APMSensorsComponentController::_handleUASTextMessage);
...
void APMSensorsComponentController::_stopCalibration(APMSensorsComponentController::StopCalibrationCode code)
{
    ...
    (void) disconnect(_vehicle, &Vehicle::textMessageReceived, this, &APMSensorsComponentController::_handleUASTextMessage);
...
void APMSensorsComponentController::_handleUASTextMessage(int uasId, int compId, int severity, const QString &text)
{
    Q_UNUSED(compId);
    Q_UNUSED(severity);
...
bool APMSensorsComponentController::usingUDPLink()
{
    SharedLinkInterfacePtr sharedLink = _vehicle->vehicleLinkManager()->primaryLink().lock();
    if (sharedLink) {
...
void APMSensorsComponentController::_handleMagCalProgress(const mavlink_message_t &message)
{
    if (_calTypeInProgress == QGCMAVLink::CalibrationMag) {
        ...
        if (_progressBar) {
            _progressBar->setProperty("value", static_cast<float>(_rgCompassCalProgress[0] + _rgCompassCalProgress[1] + (_rgCompassCalProgress[2]) / 100.0));
        }
    }
}
    ...
    if (commandLong.command == MAV_CMD_ACCELCAL_VEHICLE_POS) {
        ...
        case ACCELCAL_VEHICLE_POS_LEFT:
            if (!_orientationCalLeftSideInProgress) {
                ...
                _progressBar->setProperty("value", static_cast<qreal>(17 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_RIGHT:
            if (!_orientationCalRightSideInProgress) {
                ...
                _progressBar->setProperty("value", static_cast<qreal>(34 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_NOSEDOWN:
            if (!_orientationCalNoseDownSideInProgress) {
                ...
                _progressBar->setProperty("value", static_cast<qreal>(51 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_NOSEUP:
            if (!_orientationCalTailDownSideInProgress) {
                ...
                _progressBar->setProperty("value", static_cast<qreal>(68 / 100.0));
            }
            break;
        case ACCELCAL_VEHICLE_POS_BACK:
            if (!_orientationCalUpsideDownSideInProgress) {
                ...
                _progressBar->setProperty("value", static_cast<qreal>(85 / 100.0));
            }
            break;
