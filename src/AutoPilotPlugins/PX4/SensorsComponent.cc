...
bool SensorsComponent::setupComplete(void) const
{
    for (const QString &triggerParam : std::as_const(_deviceIds)) {
        if (_vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, triggerParam).rawValue().toFloat() == 0.0f) {
            return false;
        }
    }
    bool magEnabled = true;
    if (_vehicle->parameterManager()->parameterExists(ParameterManager::anyComponentId, _magEnabledParam)) {
        magEnabled = _vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, _magEnabledParam).rawValue().toBool();
    }

    if (magEnabled && _vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, _magCalParam).rawValue().toFloat() == 0.0f) {
        return false;
    }

    if (_vehicle->fixedWing() || _vehicle->vtol() || _vehicle->airship()) {
        if (_vehicle->firmwareMajorVersion() > 1 || (_vehicle->firmwareMajorVersion() == 1 && _vehicle->firmwareMinorVersion() > 14)) {
            if (_vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, "SYS_HAS_NUM_ASPD").rawValue().toBool() &&
                    _vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, "SENS_DPRES_OFF").rawValue().toFloat() == 0.0f) {
                return false;
            }
        } else {
            if (!_vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, "FW_ARSP_MODE").rawValue().toBool() &&
                    _vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, "CBRK_AIRSPD_CHK").rawValue().toInt() != 162128 &&
                    _vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, "SENS_DPRES_OFF").rawValue().toFloat() == 0.0f) {
                return false;
            }
        }
...
QStringList SensorsComponent::sections() const
{
    ...
    if (_vehicle->parameterManager()->parameterExists(ParameterManager::anyComponentId, _magEnabledParam)) {
        allMagsDisabled = !_vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, _magEnabledParam).rawValue().toBool();
    }
    if (!allMagsDisabled) {
...
QUrl SensorsComponent::summaryQmlSource(void) const
{
 ...
 {
    return _airspeedCalSupported() && _vehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, "SENS_DPRES_OFF").rawValue().toFloat() == 0.0f;
 }
...
bool SensorsComponent::sectionSetupComplete(const QString &sectionName) const
{
    ...
    if (sectionName == tr("Compass")) {
        bool magEnabled = true;
        if (pm->parameterExists(ParameterManager::anyComponentId, _magEnabledParam)) {
            magEnabled = pm->requireParameter(ParameterManager::anyComponentId, _magEnabledParam).rawValue().toBool();
        }
        if (!magEnabled) return true;
        return pm->requireParameter(ParameterManager::anyComponentId, _magCalParam).rawValue().toFloat() != 0.0f;
    }
    if (sectionName == tr("Gyroscope")) {
        return pm->requireParameter(ParameterManager::anyComponentId, "CAL_GYRO0_ID").rawValue().toFloat() != 0.0f;
    }
    if (sectionName == tr("Accelerometer")) {
        return pm->requireParameter(ParameterManager::anyComponentId, "CAL_ACC0_ID").rawValue().toFloat() != 0.0f;
    }
    if (sectionName == tr("Level Horizon")) {
        ...
        return pm->requireParameter(ParameterManager::anyComponentId, "CAL_ACC0_ID").rawValue().toFloat() != 0.0f
            && pm->requireParameter(ParameterManager::anyComponentId, "CAL_GYRO0_ID").rawValue().toFloat() != 0.0f;
    }
