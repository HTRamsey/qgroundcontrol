...
SettingsGroupLayout {
    ...
    LabelledLabel {
        label:     qsTr("Error")
        labelText: root.gps ? root._gpsErrorText(root.gps.systemErrors.value) : ""
    ...
    LabelledLabel {
        ...
        labelText: {
            if (!root.gps) return root.valueNA
            var h = isNaN(root.gps.hAcc.rawValue) ? root.valueNA : root.gps.hAcc.valueString
            var v = isNaN(root.gps.vAcc.rawValue) ? root.valueNA : root.gps.vAcc.valueString
            return h + " / " + v
        }
