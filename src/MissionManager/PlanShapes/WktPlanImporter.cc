...
WktPlanImporter::WktPlanImporter(QObject* parent)
    : PlanImporter(parent)
{
    registerImporter(QStringLiteral("wkt"), this);
}
...
PlanImportResult WktPlanImporter::importFromFile(const QString& filename)
{
    ...
    if (result.success) {
        result.formatDescription = tr("WKT (%1 geometries)").arg(result.itemCount());
    } else {
        result.errorString = errorString.isEmpty() ? tr("No valid WKT geometries found") : errorString;
    }
