...
double GeoFenceController::paramCircularFence(void)
{
    ...
    if(_managerVehicle->px4Firmware()){
        if(!_managerVehicle->parameterManager()->parameterExists(ParameterManager::anyComponentId, _px4ParamCircularFence)){
            return 0;
        }

        return _managerVehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, _px4ParamCircularFence).rawValue().toDouble();
    }

    if(_managerVehicle->apmFirmware())
    {
        if (!_managerVehicle->parameterManager()->parameterExists(ParameterManager::anyComponentId, _apmParamCircularFenceRadius) ||
            !_managerVehicle->parameterManager()->parameterExists(ParameterManager::anyComponentId, _apmParamCircularFenceEnabled) ||
            !_managerVehicle->parameterManager()->parameterExists(ParameterManager::anyComponentId, _apmParamCircularFenceType)){
            return 0;
        }

        bool apm_fence_enabled = _managerVehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, _apmParamCircularFenceEnabled).rawValue().toBool();
        bool apm_fence_type_circle = (1 << 1) & _managerVehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, _apmParamCircularFenceType).rawValue().toUInt();

        if(!apm_fence_enabled || !apm_fence_type_circle)
            return 0;

        return _managerVehicle->parameterManager()->requireParameter(ParameterManager::anyComponentId, _apmParamCircularFenceRadius).rawValue().toDouble();
    }
...
void GeoFenceController::_parametersReady(void)
{
    ...
    if(_managerVehicle->px4Firmware()){
        if(!_paramManager->parameterExists(ParameterManager::anyComponentId, _px4ParamCircularFence)){
            emit paramCircularFenceChanged();
            return;
        }

        _px4ParamCircularFenceFact = _paramManager->getParameter(ParameterManager::anyComponentId, _px4ParamCircularFence);
    ...
    {
        if (!_paramManager->parameterExists(ParameterManager::anyComponentId, _apmParamCircularFenceRadius) ||
            !_paramManager->parameterExists(ParameterManager::anyComponentId, _apmParamCircularFenceEnabled) ||
            !_paramManager->parameterExists(ParameterManager::anyComponentId, _apmParamCircularFenceType)){
            emit paramCircularFenceChanged();
            return;
        }

        _apmParamCircularFenceRadiusFact = _paramManager->getParameter(ParameterManager::anyComponentId, _apmParamCircularFenceRadius);
        _apmParamCircularFenceEnabledFact = _paramManager->getParameter(ParameterManager::anyComponentId, _apmParamCircularFenceEnabled);
        _apmParamCircularFenceTypeFact = _paramManager->getParameter(ParameterManager::anyComponentId, _apmParamCircularFenceType);
