...
bool JoystickSDL::init()
{
    ...
    if (SDL_InitSubSystem(SDL_INIT_GAMECONTROLLER | SDL_INIT_JOYSTICK) < 0) {
        ...
        qCWarning(JoystickLog) << "Failed to initialize SDL:" << SDL_GetError();
        return false;
    }
...
QMap<QString, Joystick*> JoystickSDL::discover()
{
    ...
    qCDebug(JoystickLog) << "Discovering joysticks";
    ...
    for (int i = 0; i < SDL_NumJoysticks(); i++) {
        ...
        if (!sdlJoystick) {
            qCWarning(JoystickLog) << "SDL failed opening joystick" << qPrintable(name) << "error:" << SDL_GetError();
            continue;
        }
        ...
        if ((axisCount < 0) || (buttonCount < 0) || (hatCount < 0)) {
            qCWarning(JoystickLog) << "SDL error parsing joystick features:" << SDL_GetError();
        }
...
bool JoystickSDL::_open()
{
    ...
    qCDebug(JoystickLog) << "Opened" << SDL_JoystickName(_sdlJoystick) << "joystick at" << _sdlJoystick;
...
{
    if (!_sdlJoystick) {
        qCWarning(JoystickLog) << "Attempt to close null joystick!";
        return;
    }

    qCDebug(JoystickLog) << "Closing" << SDL_JoystickName(_sdlJoystick) << "joystick at" << _sdlJoystick;
...
void JoystickSDL::_loadGameControllerMappings()
{
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qCWarning(JoystickLog) << "Couldn't load GameController mapping database.";
        return;
    }

    QTextStream stream(&file);
    while (!stream.atEnd()) {
        SDL_GameControllerAddMapping(stream.readLine().toStdString().c_str());
    }
}
