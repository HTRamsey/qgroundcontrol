...
void QGCCacheWorker::_pruneCache(QGCMapTask* mtask)
{
    ...
    if (query.exec(s)) {
        ...
        while (tlist.count()) {
            s = QStringLiteral("DELETE FROM Tiles WHERE tileID = %1").arg(tlist.first());
            tlist.removeFirst();
            if (!query.exec(s)) {
                break;
            }
...
bool QGCCacheWorker::_createDB(QSqlDatabase& db, bool createDefault)
{
    ...
    if (res && createDefault) {
        const QString s = QStringLiteral("SELECT name FROM TileSets WHERE name = \"%1\"").arg(kDefaultSet);
        if (query.exec(s)) {
            if (!query.next()) {
