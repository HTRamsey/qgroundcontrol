...
void trackAndCreateParentDirs(const QString &path, const QString &outputDir,
                               QSet<QString> &existingDirs, QStringList &createdDirs)
{
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if (fileType == AE_IFLNK) {
            ...
            if (symlinkTarget) {
                ...
                if (!resolvedTarget.startsWith(canonicalOutputDir + "/") &&
                    resolvedTarget != canonicalOutputDir) {
                    qCWarning(QGClibarchiveLog) << "Skipping symlink escaping output directory:"
                                                << currentFile << "->" << symlinkTarget;
                    continue;
                }
            }
        }
...
bool openArchiveForReading(struct archive *a, const QString &filePath, QByteArray &resourceData)
{
    ...
    if (isResource) {
        ...
        if (archive_read_open_memory(a, resourceData.constData(),
                                      static_cast<size_t>(resourceData.size())) != ARCHIVE_OK) {
...
bool validateArchive(const QString &archivePath)
{
    ...
    while (archive_read_next_header(reader.handle(), &entry) == ARCHIVE_OK) {
        ...
        if (size < 0) {
            qCWarning(QGClibarchiveLog) << "Validation failed for entry:"
...
ArchiveStats getArchiveStats(const QString &archivePath)
{
    ...
    qCDebug(QGClibarchiveLog) << "Archive stats:" << stats.fileCount << "files,"
