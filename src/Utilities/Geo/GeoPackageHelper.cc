...
bool createGeoPackage(const QString &filePath, QString &errorString)
{
    ...
    if (QFile::exists(filePath)) {
        if (!QFile::remove(filePath)) {
            errorString = QObject::tr("Cannot remove existing file");
            return false;
        }
    }
...
{
    if (points.isEmpty()) {
        errorString = QObject::tr("No points to save");
        return false;
    }
    ...
    for (const QGeoCoordinate &point : points) {
        ...
        if (!query.exec()) {
            errorString = QObject::tr("Failed to insert point: %1").arg(query.lastError().text());
            closeDatabase(db);
            return false;
        }
    }
                   ...
                   const QString &tableName, QString &errorString)
{
    if (polylines.isEmpty()) {
        errorString = QObject::tr("No polylines to save");
        return false;
    }
    ...
    for (const QList<QGeoCoordinate> &coords : polylines) {
        ...
        if (!query.exec()) {
            errorString = QObject::tr("Failed to insert polyline: %1").arg(query.lastError().text());
            closeDatabase(db);
            return false;
        }
    }
...
{
    if (polygons.isEmpty()) {
        errorString = QObject::tr("No polygons to save");
        return false;
    }
    ...
    for (const QList<QGeoCoordinate> &vertices : polygons) {
        ...
        if (!query.exec()) {
            errorString = QObject::tr("Failed to insert polygon: %1").arg(query.lastError().text());
            closeDatabase(db);
            return false;
        }
    }
