...
QGC_LOGGING_CATEGORY(LogManagerLog, "Utilities.Logging.LogManager", QtWarningMsg)
...
static void messageHandler(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    if (LogManager::instance()) {
        LogManager::instance()->_handleLogEntry(entry);
    }
...
void LogManager::_handleLogEntry(const LogEntry& entry)
{
    _model->append(entry);
...
{
    const QList<LogEntry> entries = _model->allEntries();

    emit exportStarted();

    QtConcurrent::run([this, destFile, entries, format]() {
        // Format content
...
{
    const QList<LogEntry> entries = _model->filteredEntries();
