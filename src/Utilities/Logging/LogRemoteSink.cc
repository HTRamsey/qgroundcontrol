...
QGC_LOGGING_CATEGORY(LogRemoteSinkLog, "Utilities.Logging.LogRemoteSink", QtWarningMsg)
...
void LogRemoteSink::setEndpoint(const QString& hostPort)
{
...

    {
        ...
        if (!hostStr.isEmpty() && !portStr.isEmpty()) {
            _host = QHostAddress(hostStr);
            _port = portStr.toUShort();
            host = _host;
            port = _port;
            ...
            _host.clear();
            _port = 0;
            qCWarning(LogRemoteSinkLog) << "Invalid endpoint format:" << hostPort;
        }
    }
...
void LogRemoteSink::_handleUdpFailure()
{
    ...
    bool alreadyFallback;
    {
        ...
        threshold = _udpFailureThreshold;
        alreadyFallback = _usingTcpFallback;
    }

    if (failures >= threshold && !alreadyFallback) {
...
{
    Q_UNUSED(bytes);
    // Reset UDP failure counter on successful send
    QMutexLocker locker(&_mutex);
    _consecutiveUdpFailures = 0;
}
