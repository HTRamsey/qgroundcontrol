...
bool ShapeFileHelper::_fileIsKML(const QString &file, QString &errorString)
{
    ...
    if (file.endsWith(AppSettings::kmlFileExtension)) {
        ...
        errorString = QString(_errorPrefix).arg(tr("Unsupported file type. Only .%1 and .%2 are supported.").arg(AppSettings::kmlFileExtension, AppSettings::shpFileExtension));
    }
...
{
    return QStringList(tr("KML/SHP Files (*.%1 *.%2)").arg(AppSettings::kmlFileExtension, AppSettings::shpFileExtension));
}
