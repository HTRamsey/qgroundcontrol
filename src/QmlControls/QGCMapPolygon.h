...
public:
    ...
    explicit QGCMapPolygon(const QGCMapPolygon &other, QObject *parent = nullptr);
    ...
    QList<QPointF> nedPolygon() const;
    ...
    // Property methods
    int count() const { return _polygonPath.count(); }
    ...
    bool isValid() const { return (_polygonModel.count() >= 3); }
    bool empty() const { return (_polygonModel.count() == 0); }
    ...
    QVariantList path() const { return _polygonPath; }
    QmlObjectListModel *qmlPathModel() { return &_polygonModel; }
    QmlObjectListModel &pathModel() { return _polygonModel; }
