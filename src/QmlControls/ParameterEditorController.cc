    ...
    , _parameterMgr(_vehicle->parameterManager())
{
    ...
    (void) connect(this, &ParameterEditorController::currentCategoryChanged, this, &ParameterEditorController::_currentCategoryChanged);
    (void) connect(this, &ParameterEditorController::currentGroupChanged, this, &ParameterEditorController::_currentGroupChanged);
    (void) connect(this, &ParameterEditorController::searchTextChanged, this, &ParameterEditorController::_searchTextChanged);
    (void) connect(this, &ParameterEditorController::showModifiedOnlyChanged, this, &ParameterEditorController::_searchTextChanged);
    (void) connect(&_searchTimer, &QTimer::timeout, this, &ParameterEditorController::_performSearch);
    (void) connect(_parameterMgr, &ParameterManager::factAdded, this, &ParameterEditorController::_factAdded);
...
void ParameterEditorController::_buildLists()
{
...
QStringList ParameterEditorController::searchParameters(const QString &searchText, bool searchInName, bool searchInDescriptions)
{
    ...
    for (const QString &paramName: _parameterMgr->parameterNames(_vehicle->defaultComponentId())) {
        if (searchText.isEmpty()) {
            ...
            if ((searchInName && fact->name().contains(searchText, Qt::CaseInsensitive)) || (searchInDescriptions && (fact->shortDescription().contains(searchText, Qt::CaseInsensitive) || fact->longDescription().contains(searchText, Qt::CaseInsensitive)))) {
                list += paramName;
            }
        }
    }
...
void ParameterEditorController::saveToFile(const QString &filename)
{
    if (!filename.isEmpty()) {
        ...
        if (!QFileInfo(filename).fileName().contains(".")) {
            parameterFilename += QStringLiteral(".%1").arg(AppSettings::parameterFileExtension);
        }
...
bool ParameterEditorController::buildDiffFromFile(const QString &filename)
{
    ...
    while (!stream.atEnd()) {
        ...
        if (_parameterMgr->parameterExists(componentId, paramName)) {
            const Fact *vehicleFact = _parameterMgr->getParameter(componentId, paramName);
            ...
            const bool vehicleRebootRequired = vehicleFactMetaData->vehicleRebootRequired();
...
void ParameterEditorController::_searchTextChanged()
{
...
void ParameterEditorController::_performSearch()
{
    const QStringList rgSearchStrings = _searchText.split(' ', Qt::SkipEmptyParts);
    ...
    for (const QString &paraName: _parameterMgr->parameterNames(_vehicle->defaultComponentId())) {
        ...
        if (matched) {
            for (const QString &searchItem : rgSearchStrings) {
                const QRegularExpression re = QRegularExpression(searchItem, QRegularExpression::CaseInsensitiveOption);
                if (re.isValid()) {
                ...
                } else if (!fact->name().contains(searchItem, Qt::CaseInsensitive) &&
...
void ParameterEditorController::_currentCategoryChanged()
{
    ParameterEditorGroup *group = nullptr;
    if (_currentCategory) {
...
void ParameterEditorController::_currentGroupChanged()
{
