...
bool FactGroup::factExists(const QString &name)
{
    if (name.contains(".")) {
        ...
        if (parts.count() != 2) {
            qcWarning(FactGroup) << "Only single level of hierarchy supported";
            return false;
        }

        const FactGroup *const factGroup = getFactGroup(parts[0]);
        if (!factGroup) {
            qcWarning(FactGroup) << "Unknown FactGroup" << parts[0];
            return false;
        }
...
Fact *FactGroup::getFact(const QString &name)
{
    ...
    if (_nameToFactMap.contains(camelCaseName)) {
        ...
        qCWarning(FactGroup) << "Unknown Fact" << camelCaseName;
    }
...
FactGroup *FactGroup::getFactGroup(const QString &name)
{
    ...
    if (_nameToFactGroupMap.contains(camelCaseName)) {
        ...
        qCWarning(FactGroup) << "Unknown FactGroup" << camelCaseName;
    }
...
void FactGroup::_addFact(Fact *fact, const QString &name)
{
    if (_nameToFactMap.contains(name)) {
        qCWarning(FactGroup) << "Duplicate Fact" << name;
        return;
    }
...
void FactGroup::_addFactGroup(FactGroup *factGroup, const QString &name)
{
    if (_nameToFactGroupMap.contains(name)) {
        qCWarning(FactGroup) << "Duplicate FactGroup" << name;
        return;
    }
...
{
    for (Fact *const fact: _nameToFactMap) {
