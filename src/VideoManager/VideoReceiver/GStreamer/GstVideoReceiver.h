...
public slots:
    void start(const QString &uri, unsigned timeout, int buffer = 0);
    void stop();
    void startDecoding(void *sink);
    void stopDecoding();
    void startRecording(const QString &videoFile, FILE_FORMAT format);
    void stopRecording();
    void takeScreenshot(const QString &imageFile);
...
private:
    ...
    static constexpr uint64_t _kUdpReconnect_us = 5 * 1000000;
