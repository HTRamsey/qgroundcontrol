...
GstVideoReceiver::GstVideoReceiver(QObject *parent)
    ...
    , _slotHandler(new GstVideoWorker(this))
{
...
void GstVideoReceiver::startDecoding(void *sink)
{
    ...
    if (_needDispatch()) {
        GstElement *const videoSink = GST_ELEMENT(sink);
...
void GstVideoReceiver::stopDecoding()
{
    ...
    // FIXME: emit after decoding has actually stopped
...
void GstVideoReceiver::startRecording(const QString &videoFile, FILE_FORMAT format)
{
    ...
    (void) gst_bin_add(GST_BIN(_pipeline), _fileSink);
...
void GstVideoReceiver::_watchdog()
{
    _slotHandler->dispatch([this]() {
        ...
        const qint64 streamInterval = now - _lastSourceFrameTime;
        if (streamInterval > _timeout) {
            qCDebug(GstVideoReceiverLog) << "Stream timeout, no frames for" << streamInterval << _uri;
            _dispatchSignal([this]() {
                emit timeout();
            });
            stop();
        ...
        if (_decoding && !_removingDecoder) {
            ...
            const qint64 decoderInterval = now - _lastVideoFrameTime;
            if (decoderInterval > (_timeout * 2)) {
                qCDebug(GstVideoReceiverLog) << "Video decoder timeout, no frames for" << decoderInterval << _uri;
                _dispatchSignal([this]() {
                    emit timeout();
                });
                stop();
...
gboolean GstVideoReceiver::_filterParserCaps(GstElement *bin, GstPad *pad, GstElement *element, GstQuery *query, gpointer data)
{
    ...
    GstStructure *const structure = gst_caps_get_structure(srcCaps, 0);
...
GstElement *GstVideoReceiver::_makeSource(const QString &uri)
{
    ...
    const bool isUdp264 = uri.contains("udp://", Qt::CaseInsensitive);
    const bool isRtsp = uri.contains("rtsp://", Qt::CaseInsensitive);
    const bool isUdp265 = uri.contains("udp265://", Qt::CaseInsensitive);
    const bool isTcpMPEGTS = uri.contains("tcp://", Qt::CaseInsensitive);
    const bool isUdpMPEGTS = uri.contains("mpegts://", Qt::CaseInsensitive);
...
GstElement *GstVideoReceiver::_makeDecoder(GstCaps *caps, GstElement *videoSink)
{
    ...
    GstElement *const decoder = gst_element_factory_make("decodebin3", nullptr);
...
GstElement *GstVideoReceiver::_makeFileSink(const QString &videoFile, FILE_FORMAT format)
{
    ...
    do {
        ...
        GstPadTemplate *const padTemplate = gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(mux), "video_%u");
        if (padTemplate) {
        ...
        GstPad *const ghostpad = gst_ghost_pad_new("sink", pad);
...
bool GstVideoReceiver::_addVideoSink(GstPad *pad)
{
    ...
    if (_decoderValve) {
        ...
        GstPad *const valveSrcPad = gst_element_get_static_pad(_decoderValve, "src");
        GstCaps *const valveSrcPadCaps = gst_pad_query_caps(valveSrcPad, nullptr);
        GstStructure *const s = gst_caps_get_structure(valveSrcPadCaps, 0);
...
{
    (void) gst_bin_remove(GST_BIN(_pipeline), _fileSink);
...
gboolean GstVideoReceiver::_onBusMessage(GstBus *bus, GstMessage *msg, gpointer data)
{
    ...
    GstVideoReceiver *const pThis = reinterpret_cast<GstVideoReceiver*>(data);
    ...
    case GST_MESSAGE_ELEMENT:
    {
        ...
        (void) gst_structure_get(s, "message", GST_TYPE_MESSAGE, &forward_msg, NULL);
...
{
    GstVideoReceiver *const self = static_cast<GstVideoReceiver*>(data);
...
gboolean GstVideoReceiver::_padProbe(GstElement *element, GstPad *pad, gpointer user_data)
{
    ...
    int *const probeRes = reinterpret_cast<int*>(user_data);
    *probeRes |= 1;
...
GstPadProbeReturn GstVideoReceiver::_keyframeWatch(GstPad* pad, GstPadProbeInfo* info, gpointer user_data)
{
    ...
    GstBuffer *const buf = gst_pad_probe_info_get_buffer(info);
...
bool GstVideoWorker::needDispatch()
{
    return (QThread::currentThread() != this);
}

void GstVideoWorker::dispatch(std::function<void()> t)
{
...
void GstVideoWorker::run()
{
    while (!_shutdown) {
        ...
        while (_taskQueue.isEmpty()) {
            (void) _taskQueueUpdate.wait(&_taskQueueSync);
        }
