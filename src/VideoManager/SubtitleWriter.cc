...
void SubtitleWriter::_captureTelemetry()
{
    ...
    static constexpr int offsetFactor = 700; // Used to simulate a larger resolution and reduce the borders in the layout
    static constexpr float nRows = 3; // number of rows used for displaying data
    static const int rowWidth = (1920 + offsetFactor) / (nRows + 1); // _size.width()
    ...
    static const QString namesLine = QStringLiteral("Dialogue: 0,%2,%3,Default,,0,0,0,,{\\an3\\pos(%1,1075)}%4\n");
    static const QString valuesLine = QStringLiteral("Dialogue: 0,%2,%3,Default,,0,0,0,,{\\pos(%1,1075)}%4\n");
