...
QGCCameraManager::CameraStruct::CameraStruct(uint8_t compID_, Vehicle *vehicle_)
    : compID(compID_)
    , vehicle(vehicle_)
{
...
{
    (void) qmlRegisterUncreatableType<MavlinkCameraControl>("QGroundControl.Vehicle", 1, 0, "MavlinkCameraControl", "Reference only");
    (void) qmlRegisterUncreatableType<QGCCameraManager>("QGroundControl.Vehicle", 1, 0, "QGCCameraManager", "Reference only");
    (void) qmlRegisterUncreatableType<QGCVideoStreamInfo>("QGroundControl.Vehicle", 1, 0, "QGCVideoStreamInfo", "Reference only");
}
...
void QGCCameraManager::_vehicleReady(bool ready)
{
    if (ready) {
        if (MultiVehicleManager::instance()->activeVehicle() == _vehicle) {
            ...
            (void) connect(JoystickManager::instance(), &JoystickManager::activeJoystickChanged, this, &QGCCameraManager::_activeJoystickChanged);
        }
    } else {
        qCDebug(CameraManagerLog) << "vehicle not ready";
    }
}
...
void QGCCameraManager::_mavlinkMessageReceived(const mavlink_message_t &message)
{
    ...
    if ((message.sysid == _vehicle->id()) && ((message.compid == MAV_COMP_ID_AUTOPILOT1) || ((message.compid >= MAV_COMP_ID_CAMERA) && (message.compid <= MAV_COMP_ID_CAMERA6)))) {
        ...
        case MAVLINK_MSG_ID_CAMERA_TRACKING_IMAGE_STATUS:
            ...
            break;
        default:
            qCWarning(CameraManagerLog) << "Unsupported Mavlink Msg ID" << message.msgid;
            break;
        }
    }
}
...
void QGCCameraManager::_handleHeartbeat(const mavlink_message_t &message)
{
    ...
    if (!_cameraInfoRequest.contains(sCompID)) {
        ...
        pInfo = new CameraStruct(message.compid, _vehicle);
        pInfo->lastHeartbeat.start();
        _cameraInfoRequest[sCompID] = pInfo;
        _requestCameraInfo(pInfo);
    } else if (_cameraInfoRequest[sCompID]) {
        pInfo = _cameraInfoRequest[sCompID];
        ...
        qCWarning(CameraManagerLog) << "_cameraInfoRequest[" << sCompID << "] is null";
    }
}
    ...
    for (int i = 0; i < _cameras.count(); i++) {
        if (_cameras[i]) {
            ...
            if (pCamera) {
                ...
                qCCritical(CameraManagerLog) << "Null MavlinkCameraControl instance";
            }
        }
    }
...
void QGCCameraManager::_handleCameraInfo(const mavlink_message_t &message)
{
    ...
    if (_cameraInfoRequest.contains(sCompID) && !_cameraInfoRequest[sCompID]->infoReceived) {
        ...
        qCDebug(CameraManagerLog) << reinterpret_cast<const char*>(info.model_name) << reinterpret_cast<const char*>(info.vendor_name) << "Comp ID:" << message.compid;
...
{
    const auto cameraInfo = static_cast<QGCCameraManager::CameraStruct*>(resultHandlerData);
...
{
    qCDebug(CameraManagerLog) << pInfo->compID;
...
void QGCCameraManager::_activeJoystickChanged(Joystick *joystick)
{
    ...
    if (_activeJoystick) {
        (void) connect(_activeJoystick, &Joystick::stepZoom,               this, &QGCCameraManager::_stepZoom);
...
void QGCCameraManager::_stepStream(int direction)
{
    ...
    if (c < 0) {
        c = (pCamera->streams()->count() - 1);
    }
    if (c >= pCamera->streams()->count()) {
        c = 0;
    }
