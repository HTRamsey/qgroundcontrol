 ...
 *			software system or to control Gimbal via some special equipments. With this SDK, developers
...
#define VLK_ERROR_TIME_OUT -2
#define VLK_ERROR_OPERATION_FAILED -3
/** @} error code */
...
typedef enum _VLK_OSD_INPUT_MASK
{
    ...
    VLK_OSD_INPUT_MASK_TIME = 0x2, ///< enable input time
    ...
    VLK_OSD_INPUT_MASK_VL_MAG = 0x20, ///< enable input VL-MAG
...
typedef struct _VLK_CHANNELS_INVERT_FLAG VLK_CHANNELS_INVERT_FLAG;
struct _VLK_CHANNELS_INVERT_FLAG
{
	char cYPMZInvertFlag;
	char cFPRMInvertFlag;
};
...
struct _VLK_DEV_CONFIG
{
	...
	VLK_CHANNELS_INVERT_FLAG ChnlInvertFlag; // 0x1A ~ 0xB
    char cReserved1[3]; ///< reserved
...
struct _VLK_DEV_TELEMETRY
{
    ...
    double sZoomMagTimes; ///< camera magnification times
...
/**
 * IP encoder video input and output
*/
typedef struct _VLK_DEV_IP_ENCODER_VIDEO_IO VLK_DEV_IP_ENCODER_VIDEO_IO;
struct _VLK_DEV_IP_ENCODER_VIDEO_IO
{
    int iOutputFrameRate;
    int iInputFrameRate;
    short sInputWidth;
    short sInputHeight;
    short sOutputWidth;
 ...
 * Video resolution
 */
typedef enum _VLK_RESOLUTION
{
    VLK_RESOLUTION_NONE,
    VLK_RESOLUTION_720P,
    VLK_RESOLUTION_1080P,
    VLK_RESOLUTION_3840x2160,
}VLK_RESOLUTION;

/**
 * Stream protocol
 */
typedef enum _VLK_STREAM_PROTOCOL
{
	VLK_SP_NONE,
	VLK_SP_UDP,
	VLK_SP_RTSP,
 ...
 * Video encode type
 */
typedef enum _VLK_VIDEO_ENCODE_TYPE
{
    VLK_VIDEO_ENCODE_NONE,
    VLK_VIDEO_ENCODE_H264,
    VLK_VIDEO_ENCODE_H265
}VLK_VIDEO_ENCODE_TYPE;

/**
 * Video code format
 */
typedef enum _VLK_VIDEO_CODE_FORMAT
{
    VLK_VIDEO_CODE_FORMAT_NONE,
    VLK_VIDEO_CODE_FORMAT_BY_SLICE,
    VLK_VIDEO_CODE_FORMAT_BY_FRAME
}VLK_VIDEO_CODE_FORMAT;

/**
 * Video save format
 */
typedef enum _VLK_VIDEO_SAVE_FORMAT
{
    VLK_VIDEO_SAVE_FORMAT_NONE,
    VLK_VIDEO_SAVE_FORMAT_MP4,
    VLK_VIDEO_SAVE_FORMAT_H264,
    VLK_VIDEO_SAVE_FORMAT_H265
}VLK_VIDEO_SAVE_FORMAT;

/**
 * IP encoder configure
*/
typedef struct _VLK_DEV_IP_ENCODER_CONFIGURE VLK_DEV_IP_ENCODER_CONFIGURE;
struct _VLK_DEV_IP_ENCODER_CONFIGURE
{
    float fBitRate;
    VLK_RESOLUTION emResolution;
    VLK_STREAM_PROTOCOL emProtocol;
    VLK_VIDEO_ENCODE_TYPE emEncodeType;
    int iFrameRate;
    VLK_VIDEO_CODE_FORMAT emCodeFormat;
    VLK_VIDEO_SAVE_FORMAT emVideoSaveFmt;
};

/**
 * IP encoder network configure
 */
typedef struct _VLK_DEV_IP_ENCODER_NETWORK VLK_DEV_IP_ENCODER_NETWORK;
struct _VLK_DEV_IP_ENCODER_NETWORK
{
    char cUDPReceiveIP[64];
    int iUDPReceivePort;
...
/**
 * IP encoder bandwidth 
 */
typedef enum _VLK_DEV_IP_ENCODER_BANDWIDTH 
{
    VLK_BANDWIDTH_SELFADAPTIVE,
    VLK_BANDWIDTH_10M
}VLK_DEV_IP_ENCODER_BANDWIDTH;

/**
 * Device status type \n
 * device status is passed by VLK_DevStatus_CB, \n
 * VLK_DEV_STATUS_TYPE will be passed to formal parameter iType, \n
...
typedef enum _VLK_DEV_STATUS_TYPE
{
    ...
    VLK_DEV_STATUS_TYPE_ENCODER_GET_VIDEO_IO_RES, ///< matches VLK_DEV_IP_ENCODER_VIDEO_IO
    VLK_DEV_STATUS_TYPE_ENCODER_GET_CFG_RES, ///< matches VLK_DEV_IP_ENCODER_CONFIGURE
	VLK_DEV_STATUS_TYPE_ENCODER_SET_CFG_RES, ///< 
    VLK_DEV_STATUS_TYPE_ENCODER_GET_VERSION_RES, ///< matches char[]
	VLK_DEV_STATUS_TYPE_ENCODER_SET_NETWORK_RES, ///< 
	VLK_DEV_STATUS_TYPE_ENCODER_SET_BANDWITH_RES,
	VLK_DEV_STATUS_TYPE_ENCODER_SET_DHCP_RES,
 ...
 *  @param iError error code, VLK_ERROR_NO_ERROR on success, error code otherwise
 *  @param szBuffer device status data, it is an address of a struct matches iType, e.g: VLK_DEV_MODEL
 ...
 */
typedef int (*VLK_DevStatus_CB)(int iType, int iErrorCode, const char* szBuffer, int iBufLen, void* pUserParam);
...
/** @brief Set device time zone
 *  @param cTZ e.g: 8 (Beijing), 9 (Seoul)
 */
...
/** @brief Set wireless control channels map and reverse configure
*  @param pChnlsMap wireless control channels map configuration
*  @param pChnlInvertFlag channels reverse configuration 
*  @details we don't provide VLK_GetWirelessCtrlChnlMap because channels map is included in device configuration
*  @see VLK_CHANNELS_INVERT_FLAG
*/
...
/** @} IP Encoder Configure functions */
