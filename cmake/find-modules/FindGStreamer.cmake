...
if(WIN32)
    ...
    find_program(PKG_CONFIG_PROGRAM pkg-config PATHS ${GSTREAMER_PREFIX}/bin REQUIRED)
    ...
    if(MSVC)
        list(APPEND PKG_CONFIG_ARGN --msvc-syntax)
    endif()
elseif(MACOS)
    ...
    find_program(PKG_CONFIG_PROGRAM pkg-config PATHS ${GSTREAMER_PREFIX}/bin REQUIRED NO_DEFAULT_PATH)
    set(ENV{PKG_CONFIG} "${PKG_CONFIG_PROGRAM}")
    find_package(PkgConfig REQUIRED)
    set(ENV{PKG_CONFIG_PATH} "${GSTREAMER_PREFIX}/lib/pkgconfig:${GSTREAMER_PREFIX}/lib/gstreamer-1.0/pkgconfig:$ENV{PKG_CONFIG_PATH}")
elseif(LINUX)
...
elseif(ANDROID)
    ...
    if(CMAKE_HOST_WIN32)
        set(ENV{PATH} "${GSTREAMER_PREFIX}/share/gst-android/ndk-build/tools/windows;$ENV{PATH}")
...
# Use Latest Revisions for each minor version: 1.16.3, 1.18.6, 1.20.7, 1.22.12, 1.24.11
...
function(find_gstreamer_component component pkgconfig_name)
    ...
    if(NOT TARGET ${target})
        ...
        pkg_check_modules(PC_GSTREAMER_${upper} IMPORTED_TARGET ${pkgconfig_name}) # >=${GStreamer_VERSION}
