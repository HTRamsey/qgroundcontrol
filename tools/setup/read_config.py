...
def format_github_output(values: dict[str, str]) -> str:
    ...
    if "QT_MODULES" in values:
        # iOS excludes qtserialport and qtscxml
        ios_modules = values["QT_MODULES"]
        for exclude in ["qtserialport", "qtscxml"]:
            ios_modules = ios_modules.replace(exclude, "")
        ios_modules = " ".join(ios_modules.split())  # Normalize whitespace
