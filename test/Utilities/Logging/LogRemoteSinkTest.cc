...
void LogRemoteSinkTest::_testBatchSend()
{
    ...
    // Send multiple entries that should be batched
    for (int i = 0; i < 5; ++i) {
        LogEntry entry;
        entry.timestamp = QDateTime::currentDateTime();
    ...
    // Should be a batch with "logs" array
    QVERIFY(json.contains("vid"));
    QCOMPARE(json["vid"].toString(), QString("batch-test-vehicle"));
    QVERIFY(json.contains("logs"));
    ...
    // Verify each entry
    for (int i = 0; i < 5; ++i) {
        QJsonObject entry = logs[i].toObject();
        QVERIFY(entry["msg"].toString().contains(QString("Batch message %1").arg(i)));
    }
}
...
void LogRemoteSinkTest::_testBatchSizeLimit()
{
    ...
    int totalReceived = 0;
    while (_udpReceiver->waitForReadyRead(500) && totalReceived < 6) {
...
void LogRemoteSinkTest::_testTcpSend()
{
    ...
    QVERIFY(_tcpServer->waitForNewConnection(3000));
