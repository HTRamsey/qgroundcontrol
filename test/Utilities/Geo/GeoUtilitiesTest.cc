...
void GeoUtilitiesTest::_testIsValidAltitudeCoordinate()
{
    ...
    QGeoCoordinate highAlt(47.0, -122.0, 50000.0);
...
void GeoUtilitiesTest::_testValidateAltitudes()
{
    ...
    invalidCoords.append(QGeoCoordinate(48.0, -121.0, 50000.0));  // Out of range
...
void GeoUtilitiesTest::_testFuzzKMLParser()
{
    ...
    for (const auto &input : fuzzInputs) {
        ...
        if (tempFile.open()) {
            ...
            QString error;
            QList<QGeoCoordinate> coords;
            ShapeFileHelper::loadPolygonFromFile(tempFile.fileName(), coords, error);
        }
    }
