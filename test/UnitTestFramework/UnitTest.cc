...
namespace {
    ...
    constexpr int kLongTimeoutMs = 30000;  // 30 second timeout for vehicle operations
}
...
void UnitTest::_connectMockLink(MAV_AUTOPILOT autopilot, MockConfiguration::FailureMode_t failureMode)
{
    ...
    QVERIFY2(spyVehicle.wait(kLongTimeoutMs), "Timeout waiting for vehicle connection");
    ...
    if (autopilot != MAV_AUTOPILOT_INVALID) {
        ...
        QVERIFY2(spyConnect.wait(kLongTimeoutMs), "Timeout waiting for initial connect");
    }
}
...
void UnitTest::_disconnectMockLink()
{
    if (_mockLink) {
        ...
        QVERIFY2(spyVehicle.wait(kLongTimeoutMs), "Timeout waiting for vehicle disconnection");
        _vehicle = MultiVehicleManager::instance()->activeVehicle();
